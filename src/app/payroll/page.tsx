...
export default function PayrollPage() {
  ...
  const calculatePayslip = async () => {
    ...
    try {
      ...
      const attHistory = await attendanceApi
        .getHistory({
      ...
      if (attRecords.length === 0) {
        setError(
          `Chưa có dữ liệu chấm công cho tháng ${MONTHS[selectedMonth - 1]} ${selectedYear}`,
        );
        setPayslip(null);
        setLoading(false);
        return;
      ...
      const workedDays = attRecords.filter(
        (r: any) => r.status === "PRESENT" || r.checkInTime,
      ).length;

      // Fetch overtime (approved) for the month
      const otRes = await overtimeApi
        .getRequests({
          employeeId: isHROrAdmin ? emp.id : undefined,
          status: "approved",
          startDate,
      ...
      const unpaidLeaveDays = leaveRecords
        .filter((r: any) => {
          ...
          return (
            start.getMonth() + 1 === selectedMonth &&
  ...
  const totalDeductions = payslip
    ? payslip.totalInsurance + payslip.incomeTax
    : 0;
            ...
            <CardContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={4}>
                  ...
                  <Typography variant="body1" fontWeight="medium">
                    {payslip.workedDays}/{STANDARD_DAYS} ngày (
                    {((payslip.workedDays / STANDARD_DAYS) * 100).toFixed(0)}%)
                  </Typography>
                </Grid>
              </Grid>
            </CardContent>
              ...
              <Card elevation={2} sx={{ height: "100%" }}>
                <CardContent>
                  ...
                  <Stack spacing={2}>
                      ...
                      <Box>
                        ...
                        >
                          ...
                          >
                            +
                            {formatVND(
                              (payslip.baseSalary / STANDARD_DAYS / 8) *
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            -
                            {formatVND(
                              (payslip.baseSalary / STANDARD_DAYS) *
                    ...
                    {/* Total Gross Pay */}
                    <Box
                      sx={{ p: 1.5, bgcolor: "success.50", borderRadius: 1 }}
                    >
                      <Box
                        sx={{
                          display: "flex",
                      ...
                      <Box>
                        ...
                        >
                          ...
                          >
                            ...
                            <Tooltip
                              title={`Thu nhập chịu thuế: ${formatVND(payslip.taxableIncome)}`}
                            >
                              <InfoIcon
                                fontSize="small"
                                sx={{ color: "text.secondary", cursor: "help" }}
                        ...
                        <Typography
                          ...
                          sx={{ pl: 4 }}
                        >
                          10% × (Thu nhập gộp - Bảo hiểm - Giảm trừ{" "}
                          {formatVND(PERSONAL_DEDUCTION)})
                        </Typography>
                        <LinearProgress
                          variant="determinate"
                          value={10}
                  ...
                  <TableBody>
                    ...
                    {payslip.overtimeHours > 0 && (
                      <TableRow>
                        ...
                        <TableCell align="right">
                          {payslip.overtimeHours.toFixed(1)} giờ
                        </TableCell>
                        <TableCell align="right">
                          {formatVND(
                            (payslip.baseSalary / STANDARD_DAYS / 8) *
                    ...
                    <TableRow sx={{ bgcolor: "grey.50" }}>
                      <TableCell colSpan={2}>
                        <strong>Tổng thu nhập gộp</strong>
                      </TableCell>
                      <TableCell align="right">
                        <strong>{formatVND(payslip.grossPay)}</strong>
                      </TableCell>
                    </TableRow>
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={3}
                        sx={{ bgcolor: "error.50", fontWeight: "bold" }}
                      >
                    ...
                    {payslip.incomeTax > 0 && (
                      <TableRow>
                        ...
                        <TableCell align="right">
                          10% × ({formatVND(payslip.taxableIncome)})
                        </TableCell>
                        <TableCell align="right">
                          -{formatVND(payslip.incomeTax)}
                        </TableCell>
                    ...
                    <TableRow sx={{ bgcolor: "grey.50" }}>
                      ...
                      <TableCell align="right">
                        <strong>-{formatVND(totalDeductions)}</strong>
                      </TableCell>
                    </TableRow>
